...
public class MenuScreen extends ScreenAdapter {
    ...
    public MenuScreen(AssetManager game) {
    ...
    public void render(float delta) {
        ...
        for (int i = 0; i < menuStrings.length; i++) {
            ...
            menuFont.draw(batch, menuStrings[i], offsetLeft, AssetManager.GAME_HEIGHT - offsetTop - i * offsetY);
        }
        batch.end();
    }
        ...
        if (Gdx.input.justTouched()) {
            ...
            for (int i = 0; i < menuStrings.length; i++) {
                if (touchWorldCoords.x > offsetLeft) {
                    float pos = AssetManager.GAME_HEIGHT - offsetTop - i * offsetY;
